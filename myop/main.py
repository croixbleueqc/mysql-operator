...
import kopf
from . import settings as myop_settings
...
def configure(settings: kopf.OperatorSettings, **_):
    ...
    """
    # watching
    settings.watching.server_timeout = myop_settings.WATCHING_SERVER_TIMEOUT
    settings.watching.connect_timeout = myop_settings.WATCHING_CONNECT_TIMEOUT
    ...
    # Admission
    host = os.environ.get(myop_settings.ENV_HOST)
    if host is not None:
    ...
    settings.admission.managed = myop_settings.DEFAULT_ADMISSION_MANAGED

    # Storage
    settings.persistence.finalizer = f'{myop_settings.DOMAIN}/kopf-finalizer'
    settings.persistence.progress_storage = kopf.StatusProgressStorage(name=myop_settings.OPERATOR)
    settings.persistence.diffbase_storage = kopf.AnnotationsDiffBaseStorage(
        prefix=myop_settings.DOMAIN,
